...
class Instamojo {
  ...
  private $_currencies = array("INR", "USD");
  ...
  public function __construct($app_id, $username = null, $password = null)
  {
    ...
    {
      throw new Exception("$app_id is required.", 1);
    }
  ...
  public function apiAuth()
  {
    ...
    if ($response['errno'])
    {
      throw new Exception("Exception: " . $response['error']);
    }
    if (!$json["success"])
    {
      throw new Exception("Application authentication failed. Check credentials");
    }
  ...
  public function listAllOffers()
  {
    if (!$this->AUTH_TOKEN)
    {
      throw new Exception("Please authenticate your application.");
    }
    $response = $this->apiRequest('offer/', 'GET');
    $json = @json_decode($response['response'], true);
    if (!$json['success'])
    {
      throw new Exception("Error in listing all offers.");
    }
    return $json;
  }
  ...
  public function listOneOfferDetail($slug)
  {
    ...
    if (!$json['success'])
    {
      throw new Exception("Error in listing offer of $slug.");
    }
    return $json;
  }
  ...
  public function getUploadUrl()
  {
    ...
    if (!$json['success'])
    {
      throw new Exception("Cannot get an URL.");
    }
    return $json["upload_url"];
  }

  /**
   * Deletes the authentication token received from Instamojo
   */
  public function deleteAuthToken()
  {
    if (!$this->AUTH_TOKEN)
    {
      throw new Exception("No token loaded, unable to delete.");
    }
    $response = $this->apiRequest("auth/$this->AUTH_TOKEN/", 'DELETE');
    $json = @json_decode($response['response'], true);
    if (!$json['success'])
    {
      throw new Exception("Could not delete auth token.");
    }
    $this->AUTH_TOKEN = null;
  }

  /**
   * Archives(Deletes) the offer whose id is supplied
  ...
  public function archiveOffer($slug)
  {
    if (!$this->AUTH_TOKEN)
    {
      throw new Exception("No token loaded, unable to archive.");
    }
    $response = $this->apiRequest("offer/$slug/", 'DELETE');
    $json = @json_decode($response['response'], true);
    if (!$json['success'])
    {
     throw new Exception("Could not archive offer.");
    }
  }
  ...
  public function setTitle($title)
  {
    if (strlen($title) > 200)
    {
     throw new Exception("Title size not more than 200 allowed.");
    }
  ...
  {
    if (!$this->allowed_currency($currency))
    {
  ...
  {
    if (!(is_numeric($base_price) && (int) $base_price >= 0))
    {
      throw new Exception("The base_price should be a positive number or zero.");
    }
  ...
  /**
   * A utility function to send POST request to the URL received from
   * getUploadUrl() and upload a file.
   * @param string $file_upload_url The URL received from getUploadUrl().
